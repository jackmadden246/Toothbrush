...
def lambda_handler(event, context):
    ...
    df['Delivery Postcode'] = (df['Delivery Postcode'].str.replace(whitespace_re, ' ')).where( df['Delivery Postcode'].str.contains(whitespace_re),
                                                                                              df['Delivery Postcode'])
    df['Billing Postcode'] = df['Billing Postcode'].astype(str)
    ...
    df.rename(columns ={'Dispatched Date':'Dispatch_Date', 'Order Date': 'Order_Date' , 'Delivery Postcode': 'Delivery_Postcode', 'Order Number': 'Order_Number', 
                        'Toothbrush Type': 'Toothbrush_Type', 'Customer Age': 'Customer_Age', 'Order Quantity':'Order_Quantity', 
